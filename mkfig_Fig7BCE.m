...
% parameters to vary
RLparas = [0.5, 5, 1]; % [alpha (learning rate), beta (inverse temperature), gamma (time discount factor)]
ds = [0:0.0001:0.0474 0.04741:0.00001:0.0480 0.0481:0.0001:0.0555 0.05551:0.00001:0.0561 0.0562:0.0001:0.1]; % set of decay-degree
...
A = axes;
hold on;
P = plot([ds(1) ds(end)],[0.5 0.5],'k:');
for k1 = 1:length(ds)
    for kk = 1:length(fzero_init_indice{k1})
        k2 = fzero_init_indice{k1}(kk);
        %P = plot(ds(k1),P1s(k1,k2),['b' Symbols_for_plot{Types_of_equibriums(k1,k2)}]);
        %P = plot(ds(k1),1-P1s(k1,k2),['r' Symbols_for_plot{Types_of_equibriums(k1,k2)}]);
        if (Types_of_equibriums(k1,k2) == 1) || (Types_of_equibriums(k1,k2) == 4)
            tmpcolor1 = 'r';
            tmpcolor2 = 'b';
            tmpsize = 15;
        ...
        P = plot(ds(k1),P1s(k1,k2),'.','color',tmpcolor1,'MarkerSize',tmpsize);
        P = plot(ds(k1),1-P1s(k1,k2),'.','color',tmpcolor2,'MarkerSize',tmpsize);
    end
end
axis([0 0.1 0 1]);
%set(A,'PlotBoxAspectRatio',[1 1 1]);
...
% Fig. 7E
plot_nullclines([0.5, 5, 1],0.0400,'Fig7E1');
plot_nullclines([0.5, 5, 1],0.0476,'Fig7E2');
plot_nullclines([0.5, 5, 1],0.0530,'Fig7E3');
plot_nullclines([0.5, 5, 1],0.0559,'Fig7E4');
plot_nullclines([0.5, 5, 1],0.0600,'Fig7E5');
