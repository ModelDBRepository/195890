...
load used_rand_twister_for_Fig12C
...
Dsim.rand_twister = used_rand_twister_for_Fig12C.alpha;
...
for k1 = 1:length(decay_rate_set)
    for k2 = 1:length(alpha_set)
        Dsim.Vend{k1}{k2} = zeros(num_sim,7*3);
        Dsim.Vs_whole_ave{k1}{k2} = zeros(num_trial,7*3);
        Dsim.ntsptAllbin5{k1}{k2} = zeros(num_sim,100);
        for k3 = 1:num_sim
            fprintf('alpha %d-%d-%d\n',k1,k2,k3);
            Dsim.Out{k1}{k2}{k3} = RLdecayStayGoBack(num_trial, RLtype, [alpha_set(k2),beta0,gamma0], decay_rate_set(k1), DAdep_paras);
...
Dalpha = Dsim;
save D12Calpha Dalpha
clear Dsim

% sim varying beta
%rand('twister',sum(100*clock));
%Dsim.rand_twister = rand('twister');
Dsim.rand_twister = used_rand_twister_for_Fig12C.beta;
rand('twister',Dsim.rand_twister);
...
for k1 = 1:length(decay_rate_set)
    for k2 = 1:length(beta_set)
        ...
        for k3 = 1:num_sim
            fprintf('beta %d-%d-%d\n',k1,k2,k3);
            Dsim.Out{k1}{k2}{k3} = RLdecayStayGoBack(num_trial, RLtype, [alpha0,beta_set(k2),gamma0], decay_rate_set(k1), DAdep_paras);
            Dsim.ntspt(k1,k2,k3) = length(Dsim.Out{k1}{k2}{k3}.States)/num_trial;
            Dsim.Vend{k1}{k2}(k3,:) = Dsim.Out{k1}{k2}{k3}.Vs_whole(end,:);
            Dsim.Vs_whole_ave{k1}{k2} = Dsim.Vs_whole_ave{k1}{k2} + Dsim.Out{k1}{k2}{k3}.Vs_whole/num_sim;
            Dsim.ntsptAllbin5{k1}{k2}(k3,:) = mean(reshape(diff([0;Dsim.Out{k1}{k2}{k3}.goalsteps]),5,100),1);
...
%rand('twister',sum(100*clock));
%Dsim.rand_twister = rand('twister');
Dsim.rand_twister = used_rand_twister_for_Fig12C.gamma;
rand('twister',Dsim.rand_twister);
...
for k1 = 1:length(decay_rate_set)
    for k2 = 1:length(gamma_set)
        ...
        for k3 = 1:num_sim
            fprintf('gamma %d-%d-%d\n',k1,k2,k3);
            Dsim.Out{k1}{k2}{k3} = RLdecayStayGoBack(num_trial, RLtype, [alpha0,beta0,gamma_set(k2)], decay_rate_set(k1), DAdep_paras);
            Dsim.ntspt(k1,k2,k3) = length(Dsim.Out{k1}{k2}{k3}.States)/num_trial;
            Dsim.Vend{k1}{k2}(k3,:) = Dsim.Out{k1}{k2}{k3}.Vs_whole(end,:);
            Dsim.Vs_whole_ave{k1}{k2} = Dsim.Vs_whole_ave{k1}{k2} + Dsim.Out{k1}{k2}{k3}.Vs_whole/num_sim;
            Dsim.ntsptAllbin5{k1}{k2}(k3,:) = mean(reshape(diff([0;Dsim.Out{k1}{k2}{k3}.goalsteps]),5,100),1);
...
% left
imagemax = ceil(max([max(max(Dalpha.ntspt_mean)), max(max(Dbeta.ntspt_mean)), max(max(Dgamma.ntspt_mean))]));
imagemin = 7;
F = figure;
A = axes;
hold on;
P = image(1+63*(Dalpha.ntspt_mean' - imagemin)/(imagemax - imagemin));
C = colorbar; set(C,'YTick',1+63*[3:5:48]/(imagemax-imagemin),'YTickLabel',[10:5:55]);
...
A = axes;
hold on;
P = image(1+63*(Dbeta.ntspt_mean' - imagemin)/(imagemax - imagemin));
C = colorbar; set(C,'YTick',1+63*[3:5:48]/(imagemax-imagemin),'YTickLabel',[10:5:55]);
...
A = axes;
hold on;
P = image(1+63*(Dgamma.ntspt_mean' - imagemin)/(imagemax - imagemin));
C = colorbar; set(C,'YTick',1+63*[3:5:48]/(imagemax-imagemin),'YTickLabel',[10:5:55]);
